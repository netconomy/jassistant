...
public class LinkTypeOptionProcessor implements LinkSearchOptionProcessor {
    ...
    public void processOption(Option thisOption, LinkSearchData linkSearchData, CliOutputConfiguration cliConfig, Options allOptions) {
        ...
        String linkType = "";
        String negateLinkType = "";

        if (!thisOption.getValuesList().isEmpty()) {
            linkType = thisOption.getValue(0);

            linkSearchData.setLinkType(linkType);
        } else {
            return;
        }

        if (thisOption.getValuesList().size() == 2) {
            negateLinkType = thisOption.getValue(1);

            if ("true".equalsIgnoreCase(negateLinkType.trim())) {
                linkSearchData.setNegateLinkType(true);
            }
        }
