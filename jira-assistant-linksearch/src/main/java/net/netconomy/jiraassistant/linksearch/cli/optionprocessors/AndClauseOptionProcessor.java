...
public abstract class AndClauseOptionProcessor implements LinkSearchOptionProcessor {
    ...
    public void processOption(Option thisOption, LinkSearchData linkSearchData, CliOutputConfiguration cliConfig, Options allOptions) {
        ...
        if (!thisOption.getValuesList().isEmpty()) {
            andClause = thisOption.getValue(0);

            if (!andClause.trim().isEmpty()) {
                getProjectSearchData(linkSearchData).setAndClause(andClause.trim());
            }
        }
